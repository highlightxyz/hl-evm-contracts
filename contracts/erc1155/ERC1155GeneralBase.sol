 ...
 */
abstract contract ERC1155GeneralBase is
    ERC1155Base,
    ...
    MarketplaceFiltererAbridged
{
    ...
    function mintOneToMultipleRecipients(address[] calldata recipients) external onlyMinter nonReentrant {
        ...
        for (uint256 i = 0; i < recipientsLength; i++) {
            tempSupply++;
            _mint(recipients[i], tempSupply, 1, "");
        }
    ...
    /**
     * @notice See {IERC1155GeneralMint-mintSpecificTokenToOneRecipient}
     */
    ...
    /**
     * @notice See {IERC1155GeneralMint-mintSpecificTokensToOneRecipient}
     */
    ...
    function burn(uint256 tokenId, uint256 amount) public nonReentrant {
        ...
        if (_manager != address(0) && IERC165Upgradeable(_manager).supportsInterface(type(IPostBurn).interfaceId)) {
            IPostBurn(_manager).postBurnAmount(msgSender, msgSender, tokenId, amount);
        } else {
            // default to restricting burn to owner or operator if a valid TM isn't present
            if (!isApprovedForAll(msgSender, msgSender)) {
                _revert(Unauthorized.selector);
            }
        }

        _burn(msgSender, tokenId, amount);
    ...
    ) internal pure virtual override(ERC1155Upgradeable, ERC1155Base, MarketplaceFiltererAbridged) {
